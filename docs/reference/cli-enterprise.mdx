...
Lists registered MCP clients and their allow lists.
...
Rotates the access token of an existing MCP client.
...
Currently this command updates the token only.

## `delete mcp-client`

Deletes an MCP client and immediately revokes its token.
...
Lists all users in the enterprise deployment.

```bash
mcpjungle list users
```

Admin accounts are shown with an `[ADMIN]` marker.

## `update user`

Rotates the access token of an existing user.
...
Deletes a user and revokes their access token immediately.
